...
export default function UniformDiscreteDistribution({ preview = false }) {
  ...
  useEffect(() => {
    ...
    // 1. ?curves=[{"a":0,"b":1,"color":"#2563eb","name":"A"},...]
    // 2. ?curve1=1,6,2563eb,A&curve2=7,9,e11d48,B (mu,sigma,color,name)
    const params = new URLSearchParams(window.location.search);
    const curvesFromUrl: UniformDiscreteCurveConfig[] = [];
    ...
    if (curvesJson) {
      try {
        ...
        if (Array.isArray(arr)) {
          for (const c of arr) {
            if (typeof c.a === 'number' && typeof c.b === 'number' && typeof c.color === 'string') {
              curvesFromUrl.push({
                a: c.a,
                b: c.b,
      ...
      while (true) {
        ...
        if (!v) break;
        const [a, b, color, ...nameArr] = v.split(',');
        if (!a || !b || !color) break;
        curvesFromUrl.push({
          a: Number(a),
          b: Number(b),
  ...
  const handleAddCurve = () => {
    ...
    setCurves([
      ...curves,
      { a: 0, b: 5, color: nextColor, name: undefined }
    ]);
  };
              ...
              <label className="flex items-center gap-1 text-xs">
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    setCurves(curves => curves.map((c, i) => {
                      if (i !== idx) return c;
                      let newA = v;
                      let newB = c.b;
                      if (newA > newB) newB = newA;
                      return { ...c, a: newA, b: newB };
                    }));
                  }}
            ...
            {svgDropdownOpen && (
              <div className="absolute z-10 right-0 mt-2 w-auto bg-white border border-gray-200 rounded shadow p-2 flex flex-row gap-2 justify-center items-center" style={{minWidth:155}}>
                <button
                  ...
                  onClick={() => {
                    if (svgString) {
                      downloadSvg('uniform-curves', svgString);
                    }
                ...
                <button
                  ...
                  onClick={() => {
                    if (cdfSvgString) {
                      downloadSvg('uniform-cdf-curves', cdfSvgString);
                    }
                    setSvgDropdownOpen(false);
                  }}
                >
                    ...
                    <div className="flex flex-row gap-2 justify-center items-center mt-2">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          await exportSvgToPng(svgString, height * 2, height, "uniform-discrete-curves");
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const height = Math.max(100, pngHeight);
                          if (cdfSvgString) {
                            await exportSvgToPng(cdfSvgString, height * 2, height, "uniform-discrete-cdf-curves");
                          }
                          setShowPngSize(false);
                        }}
                      />
         ...
         <button
          ...
          onClick={async () => {
            ...
            const params = curves.map((c, i) => {
              ...
              return `curve${i+1}=${a},${b},${color}${name ? ',' + name : ''}`;
            }).join('&');
